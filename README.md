...
- **控制台 8788**：顶部可选 Claude Code / Codex / ZCode；渠道支持搜索、筛选、测试、编辑、删除及客户端接入。
  渠道弹窗里的「模型列表」按行编辑(和多 Key 清单一个模式):一行一个模型,每行是个下拉,候选是本组
  渠道登记过的模型名;要写没登记的就选「自定义模型…」,那一行就地切成输入框。第一项作主模型,
  支持 `[1M]` 后缀;保存时按行序提交、去重、丢掉空行。它同时是「路由」页槽位表里落点模型下拉的候选。
  落点模型也是下拉(渠道模型列表 + 当前值 + 「自定义模型…」),渠道没登记的模型名照样能写进去——
  服务端槽位接口只校验字符串,不比对渠道模型表。
